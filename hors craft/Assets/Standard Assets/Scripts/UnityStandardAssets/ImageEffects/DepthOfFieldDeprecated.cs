// DecompilerFi decompiler from Assembly-CSharp-firstpass.dll class: UnityStandardAssets.ImageEffects.DepthOfFieldDeprecated
using UnityEngine;
...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class DepthOfFieldDeprecated : PostEffectsBase
	{
		...
		{
			Vector3 vector = _camera.WorldToViewportPoint((worldDist - _camera.nearClipPlane) * _camera.transform.forward + _camera.transform.position);
			return vector.z / (_camera.farClipPlane - _camera.nearClipPlane);
		}
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (simpleTweakMode)
			{
				...
				if ((bool)objectFocus)
				{
					...
					num3 = vector.z / _camera.farClipPlane;
				}
				else
				{
			...
			else
			{
				if ((bool)objectFocus)
				{
					Vector3 vector2 = _camera.WorldToViewportPoint(objectFocus.position);
					vector2.z /= _camera.farClipPlane;
					focalDistance01 = vector2.z;
				}
				else
				{
