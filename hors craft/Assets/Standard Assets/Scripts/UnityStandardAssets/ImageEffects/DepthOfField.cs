// DecompilerFi decompiler from Assembly-CSharp-firstpass.dll class: UnityStandardAssets.ImageEffects.DepthOfField
using UnityEngine;
...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class DepthOfField : PostEffectsBase
	{
		...
		{
			Vector3 vector = cachedCamera.WorldToViewportPoint((worldDist - cachedCamera.nearClipPlane) * cachedCamera.transform.forward + cachedCamera.transform.position);
			return vector.z / (cachedCamera.farClipPlane - cachedCamera.nearClipPlane);
		}
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			{
				Vector3 vector = cachedCamera.WorldToViewportPoint(focalTransform.position);
				num = vector.z / cachedCamera.farClipPlane;
			}
			else
			{
