// DecompilerFi decompiler from Assembly-CSharp.dll class: PlayerUpgradeFragment
...
public class PlayerUpgradeFragment : Fragment
{
	[Serializable]
	public class EqConfig
	{
		...
		public GameObject upgradeWindowStatGO;

		public Text upgradeWindowStatText;
	...
	private string levelTranslationText => Manager.Get<TranslationsManager>().GetText("pause.upgrades.level", "Lvl {0}");
	...
	{
		EqConfig[] array = eqConfigs;
		foreach (EqConfig eqConfig in array)
		{
			eqConfig.upgradeWindowStatText.text = eqConfig.playerStat.GetStats().ToString("F0");
		}
	}
	...
	private void UpgaradeClicked(EqConfig config)
	{
		...
		config.upgradeWindowStatText.text = " + [" + itemsUpgradeStatsSettings.GetUpgradeStats(item.id, item.level + 1, item.level).ToString() + "]";
		Array.ForEach(eqConfigs, delegate(EqConfig c)
		{
			c.upgradeWindowStatGO.SetActive(value: false);
		});
		config.upgradeWindowStatGO.SetActive(value: true);
		...
		{
			currency.TryToBuySomething(cost, delegate
			{
				achievement.ReportProgress();
				item.OnUpgrade(consumeRquired: true, backpack);
				updateWindow.SetActive(value: false);
				Fragment componentInParent = base.transform.parent.GetComponentInParent<Fragment>();
				if (componentInParent != null)
				{
					componentInParent.UpdateFragment();
				}
				else
				{
					UpdateFragment();
