// DecompilerFi decompiler from Assembly-CSharp.dll class: Uniblocks.LootableVoxelEvents
using UnityEngine;

namespace Uniblocks
{
	public class LootableVoxelEvents : DefaultVoxelEvents
	{
		...
		private void DropLoot(VoxelInfo voxelInfo)
		{
			if (Random.value < probabilityPercent / 100f)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					Vector3 lootPosition = new Vector3(x, vector.y + 0.45f, z);
