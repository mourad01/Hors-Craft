// DecompilerFi decompiler from Assembly-CSharp.dll class: GooglePlayGames.Native.NativeSavedGameClient
...
namespace GooglePlayGames.Native
{
	internal class NativeSavedGameClient : ISavedGameClient
	{
		private class NativeConflictResolver : IConflictResolver
		{
			...
			public void ResolveConflict(ISavedGameMetadata chosenMetadata, SavedGameMetadataUpdate metadataUpdate, byte[] updatedData)
			{
				...
				else
				{
					...
					mManager.Resolve(nativeSnapshotMetadata, metadataChange, mConflictId, updatedData, delegate(GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response)
					{
			...
			public void ChooseMetadata(ISavedGameMetadata chosenMetadata)
			{
				...
				{
					mManager.Resolve(nativeSnapshotMetadata, new NativeSnapshotMetadataChange.Builder().Build(), mConflictId, delegate(GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response)
					{
						if (!response.RequestSucceeded())
						{
							mCompleteCallback(AsRequestStatus(response.ResponseStatus()), null);
		...
		{
			OpenWithAutomaticConflictResolution(filename, source, resolutionStrategy, prefetchDataOnConflict: false, null, completedCallback);
		}
		...
		{
			Types.SnapshotConflictPolicy conflictPolicy;
			switch (resolutionStrategy)
			{
			case ConflictResolutionStrategy.UseLastKnownGood:
				conflictPolicy = Types.SnapshotConflictPolicy.LAST_KNOWN_GOOD;
				break;
			case ConflictResolutionStrategy.UseMostRecentlySaved:
				conflictPolicy = Types.SnapshotConflictPolicy.MOST_RECENTLY_MODIFIED;
				break;
			case ConflictResolutionStrategy.UseLongestPlaytime:
				conflictPolicy = Types.SnapshotConflictPolicy.LONGEST_PLAYTIME;
				break;
			case ConflictResolutionStrategy.UseManual:
				conflictPolicy = Types.SnapshotConflictPolicy.MANUAL;
				break;
			default:
				conflictPolicy = Types.SnapshotConflictPolicy.MOST_RECENTLY_MODIFIED;
				break;
			}
			mSnapshotManager.Open(filename, AsDataSource(source), conflictPolicy, delegate(GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse response)
			{
				if (!response.RequestSucceeded())
				{
