// DecompilerFi decompiler from Assembly-CSharp.dll class: Borodar.FarlandSkies.CloudyCrownPro.SkyboxController
using UnityEngine;
...
namespace Borodar.FarlandSkies.CloudyCrownPro
{
	...
	public class SkyboxController : MonoBehaviourSingleton<SkyboxController>
	{
		...
		private void UpdateSkyboxProperties()
		{
			if (SkyboxMaterial == null)
			{
				UnityEngine.Debug.LogWarning("SkyboxController: Skybox material is not assigned.");
				return;
			}
			SkyboxMaterial.SetColor("_TopColor", _topColor);
			SkyboxMaterial.SetColor("_BottomColor", _bottomColor);
			...
			if (_sunEnabled)
			{
				...
				if ((bool)_sunLight)
				{
					_sunLight.gameObject.SetActive(value: true);
					_sunFlareComponent = _sunLight.GetComponent<LensFlare>();
				}
				else
				{
					UnityEngine.Debug.LogWarning("SkyboxController: Sun light object is not assigned.");
				}
				if ((bool)_sunFlareComponent)
				{
			...
			else
			{
				...
				if ((bool)_sunLight)
				{
					_sunLight.gameObject.SetActive(value: false);
				}
			}
			if (_moonEnabled)
			{
				...
				if ((bool)_moonLight)
				{
					_moonLight.gameObject.SetActive(value: true);
					_moonFlareComponent = _moonLight.GetComponent<LensFlare>();
				}
				else
				{
					UnityEngine.Debug.LogWarning("SkyboxController: Moon light object is not assigned.");
				}
				if ((bool)_moonFlareComponent)
				{
			...
			else
			{
				...
				if ((bool)_moonLight)
				{
					_moonLight.gameObject.SetActive(value: false);
				}
			}
			SkyboxMaterial.SetTexture("_CloudsTex", _cloudsCubemap);
			SkyboxMaterial.SetFloat("_CloudsHeight", _cloudsHeight);
