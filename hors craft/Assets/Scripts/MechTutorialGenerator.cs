// DecompilerFi decompiler from Assembly-CSharp.dll class: MechTutorialGenerator
...
public class MechTutorialGenerator : MonoBehaviour
{
	...
	[CompilerGenerated]
	private static Action<GenericTutorial.TutorialStep, GameObject> _003C_003Ef__mg_0024cache2;
	...
	private void AddFirstStep()
	{
		...
		tutorialStep.infoDefaultText = "Hello soldier!";
	...
	private void StepOneAction()
	{
		CameraController.instance.Anchor.GetComponent<PlayerMovement>().EnableMovement(enable: false);
		CameraController.instance.InputSource.IsEnabled = false;
		dummy = (from d in UnityEngine.Object.FindObjectsOfType<TrainingDummy>()
			orderby Vector3.Distance(base.transform.position, d.transform.position)
			select d).First().gameObject;
		dummy.GetComponentInChildren<TrainingDummy>().hasToDrop = true;
		CameraController.instance.Anchor.LookAt(dummy.transform);
		Vector3 eulerAngles = CameraController.instance.Anchor.eulerAngles;
		eulerAngles.x = 0f;
		eulerAngles.z = 0f;
		CameraController.instance.Anchor.eulerAngles = eulerAngles;
		CameraController.instance.SetCameraPreset(CameraController.CameraPresets.LookAt);
		((LookAtTargetMotor)CameraController.instance.ActiveMotor).Target = dummy.transform;
		Backpack backpack = UnityEngine.Object.FindObjectOfType<Backpack>();
		backpack.TryPlace(Manager.Get<ItemsManager>().GetItem("weaponShard").prefab.GetComponent<ItemDefinition>());
		backpack.TryPlace(Manager.Get<ItemsManager>().GetItem("weaponShard").prefab.GetComponent<ItemDefinition>());
		backpack.TryPlace(Manager.Get<ItemsManager>().GetItem("weaponShard").prefab.GetComponent<ItemDefinition>());
		backpack.TryPlace(Manager.Get<ItemsManager>().GetItem("weaponShard").prefab.GetComponent<ItemDefinition>());
		AutoRefreshingStock.IncrementStockCount("free.chest");
		TimeScaleHelper.value = 0f;
		Manager.Get<SurvivalManager>().TakeCombatSemaphore();
		genericTutorial.autoChangeSteps = true;
	}
	...
	private void AddSecondStep()
	{
		...
		AddElementToClickOnSpawn(tutorialStep2, GetShootButton, AddShootingModuleCallbacks);
		tutorialStep2.tutorialCallbacks.Add(new GenericTutorial.TutorialCallback
		{
			...
			action = delegate
			{
				dummy.GetComponentInChildren<Health>().OnHit(100f, Vector3.forward);
			}
		});
	...
	private GameObject GetShootButton()
	{
		...
		private void AddFourthStep()
		{
			...
			tutorialStep2.tutorialCallbacks.Add(new GenericTutorial.TutorialCallback
			{
				...
				action = delegate
				{
					...
					StartCoroutine(WaitForIt(pathsContainer.GetValue("MechPanel")));
				}
			});
			AddTutorialStepToArray(tutorialStep2);
		}
		...
		private IEnumerator WaitForIt(string path)
		{
			...
			while (pauseTransform.GetChildByPath(path, clearFromClone: true) == null)
			{
				yield return new WaitForEndOfFrame();
			}
			genericTutorial.autoChangeSteps = true;
		...
		private GameObject GetPauseButton()
		{
			...
			private void AddFifthStep()
			{
				...
				tutorialStep.infoDefaultText = "To części twojego mecha: broń, ręce, nogi i tułów.";
				tutorialStep.shape = GenericTutorial.Shape.CUSTOM;
				GenericTutorial.TutorialStep step = tutorialStep;
			...
			private GameObject GetKnightStatsPanel()
			{
				...
				string value = pathsContainer.GetValue("MechPanel");
			...
			private void AddSixthStep()
			{
				...
				tutorialStep.infoDefaultText = "Zdobyłeś wymaganą ilość odłamków. Możesz ulepszyć swoją broń.";
				tutorialStep.shape = GenericTutorial.Shape.CUSTOM;
				GenericTutorial.TutorialStep step = tutorialStep;
			...
			private void AddThirteenthStep()
			{
				...
				tutorialStep.AddElementToStepOnSpawn(GetMenuShopButton, GenericTutorialStepHelper.AddElementButtonCallback);
				tutorialStep.infoKey = "tutorial.text.13";
				tutorialStep.infoDefaultText = "Sklep";
				AddTutorialStepToArray(tutorialStep);
			}

			private GameObject GetMenuShopButton()
			{
				return GetObjectFromPause(pathsContainer.GetValue("ShopButton"));
			}

			private void AddFourteenthStep()
			{
				GenericTutorial.TutorialStep tutorialStep = GenericTutorialStepHelper.GenerateNormalStep();
				tutorialStep.AddElementToStepOnSpawn(GetRareChestItem);
				tutorialStep.infoKey = "tutorial.text.14";
				tutorialStep.infoDefaultText = "Show Chest";
				AddTutorialStepToArray(tutorialStep);
			}

			private GameObject GetRareChestItem()
			{
				return GetObjectFromPause(pathsContainer.GetValue("RareChest"));
			}

			private void AddFifteenthStep()
			{
				GenericTutorial.TutorialStep tutorialStep = GenericTutorialStepHelper.GenerateNormalStep();
				tutorialStep.AddElementToStepOnSpawn(GetOpenChestButton, GenericTutorialStepHelper.AddElementButtonCallback);
				tutorialStep.infoKey = "tutorial.text.15";
				tutorialStep.infoDefaultText = "Open Chest";
				AddTutorialStepToArray(tutorialStep);
			}

			private GameObject GetOpenChestButton()
			{
				return GetObjectFromPause(pathsContainer.GetValue("OpenRareChest"));
			}

			private void AddSixteenthStep()
			{
				GenericTutorial.TutorialStep step = GenericTutorialStepHelper.GenerateBlockInputStep(genericTutorial);
				GenericTutorialStepHelper.ChangeToNextStepAfterDelay(genericTutorial, step, 3.1f);
				AddTutorialStepToArray(step);
			}

			private void AddSeventeenthStep()
			{
				GenericTutorial.TutorialStep tutorialStep = GenericTutorialStepHelper.GenerateNormalStep();
				tutorialStep.AddElementToStepOnSpawn(GetClaimRewardsButton, GenericTutorialStepHelper.AddElementButtonCallback);
				tutorialStep.RestoreDefaultTutorialSettingsOnInit(genericTutorial);
				tutorialStep.shape = GenericTutorial.Shape.WHOLE_SCREEN_BRIGHT;
				tutorialStep.infoKey = "tutorial.text.17";
				tutorialStep.infoDefaultText = "Claim rewards";
				AddTutorialStepToArray(tutorialStep);
			}

			private GameObject GetClaimRewardsButton()
			{
				return GetGameObjectAtPath(pathsContainer.GetValue("ClaimRewards"), Manager.Get<CanvasManager>().canvas.gameObject);
			}
			...
			private GameObject GetPotionButton()
			{
				...
				private void AddButtonCallbacksWithINitProgressButtons(GenericTutorial.TutorialStep step, GameObject buttonGO)
				{
					...
					step.tutorialCallbacks.Add(new GenericTutorial.TutorialCallback
					{
						...
						action = delegate
						{
							UnityEngine.Object.FindObjectOfType<PlayerUpgradeFragment>().InitUpgradeButtons();
							Button componentInChildren = buttonGO.GetComponentInChildren<Button>();
							componentInChildren.onClick.Invoke();
						}
					});
