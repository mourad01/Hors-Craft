// DecompilerFi decompiler from Assembly-CSharp.dll class: Spine.TrackEntry
using System;
...
namespace Spine
{
	public class TrackEntry : Pool<TrackEntry>.IPoolable
	{
		...
		internal TrackEntry SetTimelineData(TrackEntry to, ExposedList<TrackEntry> mixingToArray, HashSet<int> propertyIDs)
		{
			...
			if (to != null)
			{
				mixingToArray.RemoveAt(mixingToArray.Count - 1);
			}
			...
			for (int i = 0; i < count; i++)
			{
				...
				while (true)
				{
					if (num2 >= 0)
					{
						...
						if (!trackEntry.HasTimeline(propertyId) && trackEntry.mixDuration > 0f)
						{
							items3[i] = 3;
							items4[i] = trackEntry;
							break;
