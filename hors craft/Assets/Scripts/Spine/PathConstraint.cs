// DecompilerFi decompiler from Assembly-CSharp.dll class: Spine.PathConstraint
using System;
		...
		public void Update()
		{
			...
			if (flag5 || flag3)
			{
				...
				while (num5 < num6)
				{
					...
					exposedList.Items[++num5] = ((!flag3) ? num4 : Math.Max(0f, length + num4)) * num9 / length;
				}
			}
			else
			{
			...
			while (num13 < count)
			{
				...
				if (flag2)
				{
					...
					float c = bone3.c;
					float d = bone3.d;
					float num21 = flag4 ? array[num14 - 1] : ((exposedList.Items[num13 + 1] != 0f) ? MathUtils.Atan2(num18, num17) : array[num14 + 2]);
